                                            ...
                                            (if (or (lvar=? u (car x))
                                                ...
                                                (if (member a seen :test #'(lambda (l1 l2) (if (and (equalp (car l1) (car l2)) (equal (cadr l1) (cadr l2))) t nil)))
                                                    ...
                                                    (setq ab^ (cons a ab^)))))) ab)) ty))) (values (remove-duplicates ab^ :test #'(lambda (l1 l2) (if (and (equalp (car l1) (car l2)) (equal (cadr l1) (cadr l2))) t nil)))
                                                                                            ds)))
;(run 1 (q) (fresh (x y) (absento 'cat x) (symbolo y) (== x y) (==  `(,x ,y) q)))
;(run 1 (q) (fresh (x y) (absento 'cat x)(== x y) (symbolo y) (==  `(,x ,y) q)))
;(remove-duplicates '((#(5) cat . symbolp)
                    ;(#(4) rat . symbolp)
                    ;(#(4) rat . symbolp)) :test #'(lambda (l1 l2) (if (and (equalp (car l1) (car l2)) (equal (cadr l1) (cadr l2))) t nil)))
;(load "~/Projects/lisp/dtrace.lisp")
;(cl-user::dtrace walk-queries);post==-a<t>d/a
;(let* ((s '((#(1) #(2)) (#(2) #(3)) (#(0) . #(1))))
       ;(ty '((#(3) sym . symbolp)(#(4) sym . symbolp)))
       ;(ds '())
       ;(ab (remove nil (reform-a '((#(5) cat . symbolp)
                                  ;(#(4) rat . symbolp)
                                  ;(#(2) bat . symbolp)) s))))
 ;(post==-a<t>d/a ty ab s ds))
;(let ((s '((#(1) #(2)) (#(2) #(3)) (#(3) . cat)(#(0) #(1)))))
    ;(reform-t '((#(3) sym . symbolp)) s))

;(let ((s '((#(1) #(2)) (#(2) #(3)) (#(3) . cat)(#(0) #(1)))))
    ;(walk* #(2) s))
;(let* ((s '((#(1) #(2)) (#(2) #(3)) (#(3) . cat)(#(0) #(1))))
       ;(ty (remove nil(reform-t '((#(4) sym . symbolp)) s)))
       ;(ds '())
       ;(ab (remove nil (reform-a '((#(2) cat . symbolp)
                                   ;(#(5) bat . symbolp)
                                   ;(#(4) mat . symbolp)
                                   ;(#(6) rat . symbolp)) s))))
  ;(post==-a<t>d/a ty ab s ds))
;(let* ((s '((#(1) #(2)) (#(2) #(3)) (#(3) #(4))((#(0) #(1)))))
       ;(ty (remove nil (reform-t '((#(3) sym . symbolp)) s)))
       ;(ds '())
       ;(ab (remove nil (reform-a '((#(2) cat . symbolp)
                                   ;(#(5) bat . symbolp)
                                   ;(#(4) mat . symbolp)
