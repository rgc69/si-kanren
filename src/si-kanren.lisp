...
;(run 1 (q)
    ;(fresh (a b c d)
      ...
      ;(=/= `(,a ,b) `(,c ,d))))
      ;(== `(,a ,b ,c ,d) q)))
;(runno 1 (q) (fresh (a b)(=/= `(,a  'cat) `(12  ,b))(== q `(,a ,b))))
;(d-of (car *))
;(car *)
;(cdr *)
           ...
           (cond
             ((and (lvar? (car u)) (lvar? (cadr u))(lvar? (car v))(lvar? (cadr v)))
              (let ((sc^ (assoc (car u) TY :test #'equalp))
                ...
                (if (and (equal (tag-of sc^) (tag-of sc^^^))
                         (equal (tag-of sc^^) (tag-of sc^^^^)))
                    (unit d)
                    '(()))))
             ((and (not (lvar? (car v)))(not (lvar? (cadr v))))
              (let ((sc^ (assoc (car u) TY :test #'equalp))
                    (sc^^ (assoc (cadr u) TY :test #'equalp)))
                 (if (or sc^ sc^^)
                    (if sc^
...
;(runno 1 (q)(fresh (a b) (=/= a 3)(symbolo b)(numbero a)))
