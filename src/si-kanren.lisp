          ...
          ;(funcall (lambda (T+)
                     ;(cond ((equal T+ "same") st)
                           ...
                           ;(T (let ((d (remove nil (normalize-d<s/t/a #'subsumed-d-pr/T? (cons (car T+) (ty-of st)) (d-of st)(s-of st)))))
                                  ;;(type->diseq T+ (a-of st) d st '() '() (ty-of st)))))) ty
                                 ;(multiple-value-bind (ab ds)
                                   ;(post==-a<t>d/a (cons (car T+) (ty-of st)) (a-of st) (s-of st) d)
                                   ;(make-st
                                             ;(s/c-of st)
                                             ;ds
                                             ;(cons (car T+)(ty-of st))
    ...
    ;(cond ((lvar? u) (let ((A+ (ext-A u tag (s-of st) a)))
                       ;(cond ((null? A+) st)
                             ;(T (let ((d^ (remove nil (normalize-d<s/t/a #'subsumed-d-pr/a? (append A+ a) d (s-of st)))))
                                 ;(multiple-value-bind (ab ds)
                                   ;(post==-a<t>d/a (remove nil ty) (append A+ a) (s-of st) d^)
                                   ;(make-st
                                             ;s/c
                                             ;ds
                                             ;ty
...
;(run 1 (q) (fresh (x y)(== x y)(symbolo x)(absento 'rat y) (absento 'cat x)(== q x)))
;(run 1 (q) (fresh (x y)(symbolo x)(absento 'rat y) (absento 'cat x)(== q `(,x ,y))))
;(ext-a #(3) 'bat '((#(1) #(2))(#(0) #(1))) '((#(3) bat . symbolp) (#(4) rat . symbolp)))
;(append '(jj) nil)
;(run 1 (q) (fresh (x)(=/= 'cat x) (absento 'cat x)(== q x)))
;(runno 1 (q) (fresh (x) (absento 'cat x)(== q x)))
;(run 1 (q) (fresh (x y) (absento 'cat x) (symbolo x)(== q x)))
;(run 1 (q) (fresh (x y) (absento 'cat x) (symbolo x)(== q x)(== x 'bat)))
;(run 1 (q w) (fresh (x y) (absento 'cat x)(== w y) (numbero x)(== q x)))
;(run 1 (q w) (fresh (x y) (absento 'cat x)
             ;(== w y)
             ;(=/= x 'cat)
             ;(symbolo y)
             ;(absento 'rat y)
             ;(== q x)))
;(run 1 (q w) (fresh (x y z s) (absento 'cat x)
              ;(== w y)
              ;(=/= x 'cat)
              ;(symbolo y)
              ;(absento 'rat y)
              ...
              ;(absento 'bat s)
              ;(== q `(,x ,y ,z ,s))))
;(runno 1 (q w) (fresh (x y z s) (absento 'cat x);<--------------
                   ;(absento 'rat y)
                   ;(absento 'mat z)
                   ;(absento 'bat s)
                   ;(== w y)
                   ...
                   ;(== x y)
                   ;(== q `(,x ,y ,z ,s))))
;(run 1 (q w) (fresh (x y z s) (absento 'rat x)
                 ;(absento 'mat z)
                 ;(absento 'bat s)
                 ;(== w y)
                 ;(== z s)
                 ...
                 ;(== x y)
                 ;(=/= x 'cat) ;<-- Non toglie da =/=
                 ;;(symbolo y)  ;<-- symbolo fa casino
                 ;(absento 'cat x)
                 ;(== q `(,x ,y ,z ,s))))
;(load "~/Projects/lisp/dtrace.lisp")
;(cl-user::dtrace absento/u post==-a<t>d/a make-type-constraint/x)
;(let* ((ty nil)
       ;(ab '((#(2) cat . symbolp)))
       ;(s '((#(0) #(1))))
       ;(ds '(pluto)))
  ;(post==-a<t>d/a ty ab s ds))
;(run 1 (q) (fresh (x y z) (absento 'cat x) (=/= y 'rat) (== z y) (== q `(,x ,y ,z))))
;(run 1 (q) (fresh (x y z)(absento 'mat z) (== x y) (== z y) (== q `(,x ,y ,z))(absento 'cat x)))
